...
export const columns: ColumnDef<Item>[] = [
  ...
  },
  {
    ...
    header: 'Status',
    cell: ({ row }) => (
      <Badge className='bg-primary/10 text-primary font-normal rounded-sm px-1.5 capitalize hover:bg-primary/10'>{row.getValue('status')}</Badge>
    )
  },
  {
    accessorKey: 'paidBy',
            ...
            {table.getHeaderGroups().map(headerGroup => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map(header => {
                  return (
                    <TableHead key={header.id} className='text-muted-foreground h-10 py-2 px-2 first:pl-4'>
              ...
              table.getRowModel().rows.map(row => (
                <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                  {row.getVisibleCells().map(cell => (
                    <TableCell key={cell.id} className='first:pl-4 py-2 px-2'>
  ...
  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger asChild>
        <Button
          variant='ghost'
          className='size-8 rounded-full p-1 hover:bg-muted'
